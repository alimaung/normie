...
class ExcelDataProcessor:
    ...
    def process_verzeichnis(self, filter_values: List[str]) -> None:
        ...
        try:
            ...
            try:
                visible_cells = self.verzeichnis_sheet.Range(f'F2:F{last_row}').SpecialCells(12)
        ...
        for row in data:
            try:
                ...
                if not ats and not sdb:
                    ...
                    continue
                if sdb:
                    sdb_comm = f"ChemScan_{id_val}_{tkz}_{loc}"
                else:
                    sdb_comm = ""

                if ats:
                    ats_comm = f"AT&S_{id_val}_{tkz}_{loc}"
                else:
                    ats_comm = ""
                ...
                entry = {
                    ...
                    "sdb": sdb,
                    "loc": loc,
                    "ats_comment": ats_comm,
                    "sdb_comment": sdb_comm,
                    "exists": None,
                    "pdf": None,
                    "class": None
                }
    ...
    def cs_control(self, data: List[Dict[str, Any]]) -> None:
        ...
        for row in processed_data:
            try:
                lines = self.detect_cs_rows(row)
                for index, line in enumerate(lines, start=1):
                    entry = self.open_chem(row, index)
                    self.upload_files(entry)
                    #self.reset_browser()
        ...
        if hasattr(self, 'original_row') and self.original_row:
            self.mark_row_as_done(self.original_row, processed_data)

    def detect_cs_rows(self, data: Dict[str, Any]):
        """Detect number of rows in chemical data application"""
        ...
        try:
            ...
            logger.error(f"Error in detect_cs_rows: {e}")

    def open_chem(self, data: Dict[str, Any], index: int) -> None:
        ...
        try:
            three_dot = WebDriverWait(driver, WAIT_TIMEOUT).until(
                EC.visibility_of_element_located((By.XPATH, 
                f'/html/body/div[6]/div[2]/main/div[2]/div[3]/div[3]/div[2]/div[2]/div[2]/div/table/tbody/tr[{index}]/td[11]/div/div/a'))
            )
            ActionChains(driver).move_to_element(three_dot).perform()
            
            eye = WebDriverWait(driver, WAIT_TIMEOUT).until(
                EC.visibility_of_element_located((By.XPATH, 
                f'/html/body/div[6]/div[2]/main/div[2]/div[3]/div[3]/div[2]/div[2]/div[2]/div/table/tbody/tr[{index}]/td[11]/div/div/ul/li[2]/ul/li[4]/a'))
            )
            eye.click()
        except Exception as e:
        ...
        try:
            ...
            if nothing.text == "Keine Einträge gefunden":
                ...
                remaining = self.check_existing_entries(data)
                logger.info("Entries found in info section")
        except Exception as e:
            logger.debug(f"Could not check for entries: {e}")

    def check_existing_entries(self, data: Dict[str, Any]) -> None:
        ...
        # table body /html/body/div[6]/div[2]/main/div[2]/div[3]/div[2]/div[2]/div[2]/div/div/div[12]/div[2]/div/div/div/div/div/div/div[2]/div/div/div/div[2]/div[2]/div[4]/div/div/div/table/tbody
        # first elem /html/body/div[6]/div[2]/main/div[2]/div[3]/div[2]/div[2]/div[2]/div/div/div[12]/div[2]/div/div/div/div/div/div/div[2]/div/div/div/div[2]/div[2]/div[4]/div/div/div/table/tbody/tr[1]/td[5]
        # secon elem /html/body/div[6]/div[2]/main/div[2]/div[3]/div[2]/div[2]/div[2]/div/div/div[12]/div[2]/div/div/div/div/div/div/div[2]/div/div/div/div[2]/div[2]/div[4]/div/div/div/table/tbody/tr[2]/td[5]
        
        
        
        comment_entries = []
        for t in driver.find_elements(By.XPATH, '/html/body/div[6]/div[2]/main/div[2]/div[3]/div[2]/div[2]/div[2]/div/div/div[12]/div[2]/div/div/div/div/div/div/div[2]/div/div/div/div[2]/div[2]/div[4]/div/div/div/table/tbody'):
            string = t.text.strip(" ")
            comment = string[5]
            comment_entries.append(comment)
            
        for entry in comment_entries:
            if entry == data["ats"] or data["sdb"]:
                comment_entries.remove(entry)
    ...
    def reset_browser(self) -> None:
        ...
        try:
            ...
            # Click home button
            WebDriverWait(driver, 30).until(
                EC.visibility_of_element_located((By.XPATH, '/html/body/div[6]/div[2]/main/div[2]/div[3]/div[2]/div[1]/div/div[1]/div[1]/div/div/div/div/a'))
            )
            ...
            try:
                ...
                for key in ["ats", "sdb"]:
                    ...
                    if os.path.isfile(file_path):
                        ...
                        match = self.fuzzy_search_missing_files(os.path.basename(file_path))
                        if os.path.isfile(match):
                            row[key] = match
                            file_path = match
                            row["exists"] = True
                            logger.info(f"File {key}: {os.path.basename(match)} exists")
                            file_valid = True
                        else:
                            logger.warning(f"File {key}: {os.path.basename(match)} doesn't exist")
                    
                    # Check if file is PDF
                    if file_path.lower().endswith(".pdf") or file_path.lower().endswith(".xlsx"):
                        row["pdf"] = True
                    else:
                        row["pdf"] = False
    ...
    def weighted_similarity(self, target, candidate, weights):
        """
        Compute a weighted similarity score between two filenames.
        - `weights` is a list defining the importance of each part.
        """
        target_parts = self.split_filename(target)
        candidate_parts = self.split_filename(candidate)
    ...
    def fuzzy_search_file(self, filename, folder_path, weights, threshold=70, recursive=True):
        """
        ...
        matches = [
            (file_list[i], self.weighted_similarity(filename, filenames_only[i], weights))
    ...
    def fuzzy_search_missing_files(self, search_filename: str) -> str:
        folder = r"\\Dehesdna-a009a\projekte\k-z\ofs\Dokumentenservice\TeileundStoffe"
        ...
        matches = self.fuzzy_search_file(search_filename, folder, weights)
    ...
    def get_metadata(self, file_path: str) -> bool:
        ...
        try:
            with open(file_path, "rb") as pdf:
                ...
                if keywords != "Not found" and not None:
                    return keywords
        except Exception as e:
    ...
    # TODO: Value entering is buggy, only prints first values - fixed
    # loop
    def mark_row_as_done(self, row: int, processed_data) -> None:
        """Colors the row from column A to column D green and enters values."""
        ats_string = [] 
        sdb_string = []

        for line in processed_data:
            if line["ats_comment"] != "":
                ats = line["ats_comment"].split("_")[1].replace("-", "/")
                ats_string.append(ats)
            else: 
                continue
            if line["sdb_comment"] != "":
                sdb = line["sdb_comment"].split("_")[1].replace("-", "/")
                sdb_string.append(sdb)
            else: 
                continue
        try:
            ...
            for wb in self.excel.Workbooks:
                if wb.name == EXCEL_FILES['chemscan'].name:
                    ...
                    # Write ats and sdb id values in column C and C respectively
                    
                    active_sheet.Cells(row, 3).Value = ", ".join(ats_string)
                    active_sheet.Cells(row, 4).Value = ", ".join(sdb_string)

                    #range_to_color.Interior.Color = self._rgb_to_bgr(146, 208, 80)  # #A3F52B in RGB
    ...
    def handle_cont_loop(self, cell)-> None:
        try:
            pythoncom.CoInitialize()
            try:
                self.excel = win32com.client.GetObject(None, "Excel.Application")
                ...
                category, values = self.categorize_values(cell_data.value)
                logger.info(f"Value category: {category}")

                # Store the original row number for later coloring
                self.original_row = cell_data.row
                self.excel = self.excel  # Pass Excel reference to processor

                # Process the values if any exist
                if values:
                    self.process_verzeichnis(values)
                else:
