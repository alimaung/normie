...
class SDSParser:
    ...
    def __init__(self, fuzzy_threshold: int = 60, debug: bool = False, save_sections: bool = False):
        """
        ...
        Args:
            ...
            save_sections: Save each section to separate text files
        """
        self.fuzzy_threshold = fuzzy_threshold
        self.debug = debug
        self.save_sections = save_sections
        
    def create_output_folder(self, pdf_path: str) -> str:
        """
        Create output folder for sections based on PDF filename
        ...
        output_folder = script_dir / pdf_name
    ...
    def sanitize_filename(self, title: str) -> str:
        """
        Sanitize section title for use as filename
    ...
    def save_sections_to_files(self, sections: Dict[int, SDSSection], pdf_path: str) -> Dict[str, str]:
        """
        ...
        Returns:
            Dictionary mapping section numbers to saved file paths
        """
        if not self.save_sections:
            return {}
        ...
        try:
            ...
            output_folder = self.create_output_folder(pdf_path)
            
            if self.debug:
                print(f"Saving sections to folder: {output_folder}")
            
            # Save each found section
            for section_num, section in sections.items():
                if section.found and section.content.strip():
                    # Create filename
                    sanitized_title = self.sanitize_filename(section.title)
                    filename = f"Section_{section_num:02d}_{sanitized_title}.txt"
                    file_path = Path(output_folder) / filename
                    
                    # Save section content
                    try:
                        with open(file_path, 'w', encoding='utf-8') as f:
                            f.write(f"Section {section_num}: {section.title}\n")
                            f.write("=" * 80 + "\n\n")
                            f.write(section.content)
                        
                        saved_files[str(section_num)] = str(file_path)
                        
                        if self.debug:
                            print(f"Saved section {section_num} to: {file_path}")
                            
                    except Exception as e:
                        if self.debug:
                            print(f"Error saving section {section_num}: {e}")
            
            # Create a summary file
            summary_path = Path(output_folder) / "_summary.txt"
            try:
                with open(summary_path, 'w', encoding='utf-8') as f:
                    from datetime import datetime
                    f.write(f"SDS Parsing Summary\n")
                    ...
                    f.write(f"Sections found: {len(found_sections)}/16\n")
                    f.write(f"Found sections: {found_sections}\n")
                    if missing_sections:
                        f.write(f"Missing sections: {missing_sections}\n")
                    
                    f.write("\nSection Files:\n")
                    for section_num in sorted(found_sections):
                        if str(section_num) in saved_files:
                            file_path = Path(saved_files[str(section_num)]).name
                            f.write(f"  Section {section_num}: {file_path}\n")
    ...
    def parse_file(self, pdf_path: str) -> Dict[str, Union[bool, Dict, List]]:
        ...
        # Save sections to files if enabled
        if self.save_sections:
            saved_files = self.save_sections_to_files(sections, pdf_path)
...
def print_results(results: Union[Dict, List[Dict]], verbose: bool = False):
    ...
    for result in results:
        ...
        if 'saved_files' in result and result['saved_files']:
            print(f"📁 Sections saved to {len(result['saved_files'])} text files")
...
Examples:
  ...
  # Parse and save each section to separate text files
  python sds_parser.py --single sds.pdf --save-sections
  ...
  python sds_parser.py --batch folder --output results.json
    ...
    parser.add_argument(
        '--save-sections',
        action='store_true',
        help='Save each found section to a separate text file'
    )
    ...
    sds_parser = SDSParser(
        ...
        debug=args.debug,
        save_sections=args.save_sections
    )
