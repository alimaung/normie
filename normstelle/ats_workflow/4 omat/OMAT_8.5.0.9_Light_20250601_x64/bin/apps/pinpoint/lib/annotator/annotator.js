/**
 * Pinpoint: Customized annotator.js functions must be copied to functions when annotator.js is updated to new release.
 */
(function(f){if(typeof exports==="object"&&typeof module!=="undefined"){module.exports=f()}else if(typeof define==="function"&&define.amd){define([],f)}else{var g;if(typeof window!=="undefined"){g=window}else if(typeof global!=="undefined"){g=global}else if(typeof self!=="undefined"){g=self}else{g=this}g.annotator = f()}})(function(){var define,module,exports,scopeIdentifier;return (function e(t,n,r){function s(o,u){if(!n[o]){if(!t[o]){var a=typeof require=="function"&&require;if(!u&&a)return a(o,!0);if(i)return i(o,!0);var f=new Error("Cannot find module '"+o+"'");throw f.code="MODULE_NOT_FOUND",f}var l=n[o]={exports:{}};t[o][0].call(l.exports,function(e){var n=t[o][1][e];return s(n?n:e)},l,l.exports,e,t,n,r)}return n[o].exports}var i=typeof require=="function"&&require;for(var o=0;o<r.length;o++)s(r[o]);return s})({1:[function(require,module,exports){
(function (global){
"use strict";
...
jQuery.each( ("blur focus focusin focusout load resize scroll unload click dblclick " +
	...
	"change select submit keydown keypress keyup error contextmenu touchstart touchend").split(" "), function( i, name ) {
    ...
    SerializedRange.prototype.normalize = function(root) {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        try {
          ...
          if (!node && this[p]) {
            var array = this[p].split("/");
            if (array != null && array.length > 6) {
                array.splice(0, 6);
                var strRange = array.join("/");
                if (array.length > 0) {
                    strRange = "/" + strRange;
                }
                this[p] = strRange
...
(function() {
  ...
  Util.getFirstTextNodeNotBefore = function(n) {
    var result;
    /**
     * Pinpoint: Bug fix nodeType 'undefined'
     */
    if(typeof n !== 'undefined' && n !== 'null' && n !== 'undefined' && n.nodeType !== 'null' && n.nodeType !== 'undefined')
    {
	    switch (n.nodeType) {
...
App.prototype.start = function () {
    ...
    //var annotation = {"id":0,"title":"test","text":"TEST","quote":"temporary","ranges":[{"start":"/div[2]/div[2]/div[1]/div[1]/div[1]/span[1]","startOffset":26,"end":"/div[2]/div[2]/div[1]/div[1]/div[1]/span[1]","endOffset":37}]};
...
App.prototype.drawEmbeddedRefAnnotations = function (annotations) {
    return this.runHook('embeddedAnnotationsLoaded', [annotations]);
};
...
var id = (function () {
	/**
     * Pinpoint: Don't allow to generate internal id numbers
     */
      return function () {
        return "";
    };
...
/**
 * Pinpoint: An element from HTML template
 */
var source = '';
...
var Adder = Widget.extend({
    ...
    constructor: function (options) {
        ...
        $(this.document.body).on("touchend." + NS, function (e) {
            self._onMouseup(e);
        });

        /**
         * Pinpoint: A source of HTML template
         */
    ...
    show: function (position) {
    	/**
    	 * Pinpoint: Adder variables to calculate button and scroll position.
    	 */
        var element = this.element;
        var buttonTop = 0;
        var buttonLeft = 0;
        var scrollStart = $("#tab-" + source).scrollTop();
        var scrollStop = 0;
        var limitTop = $("#tab-" + source).offset().top;
        var limitBottom = $("#footer").length > 0 ? $("#footer").offset().top : 0;
        if (typeof position !== 'undefined' && position !== null) {
        	/**
    	 	 * Pinpoint: Adder button position before scroll event.
    	 	 */
        ...
        /**
    	 * Pinpoint: Adder listens scroll event on tab content.
    	 */
        $("#tab-" + source).scroll(function() {
            var tempStop = $("#tab-" + source).scrollTop();
            if(scrollStop !== tempStop) {
                scrollStop = tempStop;
                var moveButton = scrollStart - scrollStop;
                var pos = 0;
                if(buttonTop+moveButton < limitTop) {
                    pos = buttonTop+moveButton-limitTop;
                    element.css('top',pos);
                }
                else if (buttonTop+moveButton > limitBottom) {
                    pos = buttonTop+moveButton+window.outerHeight;
                    element.css('top',pos);
                }
                else {
                    pos = buttonTop+moveButton;
                    element.css({'top':pos,'left':buttonLeft});
                }
            }
        });
    },
    ...
    _onClick: function (event) {
        ...
        var annotationPanelScope = angular.element("[pp-annotations-panel]").scope();
        //Show alert,when trying to create annotation on symlink publication
      if (annotationPanelScope && annotationPanelScope.$root && annotationPanelScope.$root.selectedPublication && annotationPanelScope.$root.selectedPublication.sourcePublicationId) {
          annotationPanelScope.vm.showAlertForSymlinkPublication();
          return;
        }
...
var Editor = exports.Editor = Widget.extend({
    ...
    constructor: function (options) {
        ...
        this.element
            ...
            .on("click." + NS, '.annotator-organize', function (e) {
                self._onSaveClick(e);
            })
    ...
    submit: function () {
         ...
         var scopeValue = "";
         var reviewerComment ="";
         /* Pinpoint An empty field is not allowed
         */
         var scope = angular.element("#annotations-panel").scope();
        annotationTitle = this.element.find('#annotationTitle').val();
        comments = this.element.find('#comments .ql-editor')[0].innerHTML;
        scopeValue = this.element.find('input[name="scope"]:checked').attr('data-value');
        reviewerComment = this.element.find('#reviewerComment').val();

        if(!annotationTitle || annotationTitle === "" || annotationTitle.trim().length === 0) {
        	alert(scope.translate.instant('annotations.alert.typeTitle'));
        	return;
        }

        if(!comments || comments === "" || w2utils.stripTags(comments).length === 0) {
        	alert(scope.translate.instant('annotations.alert.typeComment'));
        	return;
        }

        if ((scope.vm.canWriteAnnotations() && scopeValue === "publicCandidate") && (!reviewerComment || reviewerComment === "" || reviewerComment.trim().length === 0)) {
            alert(scope.translate.instant('annotations.alert.typeApproverRejectionComments'));
            return;
        }
    ...
    addField: function (options) {
        ...
        if (field.type === 'textarea') {
            ...
            input = $('<div style="width: 300px;min-height:90px; height:90px; min-width:100%; max-height:150px"/>');
            ...
            input = $('<label type="text" id="annotator-attachment-label" style="padding-left: 8px; cursor: auto"/>');
        } else if (field.type === 'date') {
            input = $('<label type="text" id="annotator-date-label" style="padding-left: 8px; cursor: auto"/>');
        }
        ...
        if (field.type === 'radio') {
            input.addClass('annotator-radioButton');
            input.append($('<div class="scope-title">' + scope.translate.instant('annotations.title.access') + ':</div>'));
            var annotatorRadioButtons = '<div class="scope-radioButton-section">';
            var buttonList = [scope.translate.instant('annotations.title.private'), scope.translate.instant('annotations.title.organization'), scope.translate.instant('annotations.title.public'), scope.translate.instant('annotations.title.publicCandidate')];
            for (var j = 0; j < buttonList.length; j++) {
                if (buttonList[j] === scope.translate.instant('annotations.title.private')) {
                   annotatorRadioButtons = annotatorRadioButtons.concat('<div class="annotator-radioButton"><input class="annotator-radioButton radio-inline" name="scope" id="private" checked type="radio" data-value="private"/>&nbsp;' + buttonList[j] + '</div>');
                }
                if (!scope.vm.isRestrictedFragment) {
                  if (field.organizationWriteAccess && buttonList[j] === scope.translate.instant('annotations.title.organization')) {
                    annotatorRadioButtons = annotatorRadioButtons.concat('<div class="annotator-radioButton"><input class="annotator-radioButton radio-inline" name="scope" id="organization" type="radio" data-value="organization"/>&nbsp;' + buttonList[j] + '</div>');
                  }
                  if (field.publicWriteAccess && buttonList[j] === scope.translate.instant('annotations.title.public')) {
                    annotatorRadioButtons = annotatorRadioButtons.concat('<div class="annotator-radioButton"><input class="annotator-radioButton radio-inline" name="scope" id="public" type="radio" data-value="public"/>&nbsp;' + buttonList[j] + '</div>');
                  }
                  if (buttonList[j] === scope.translate.instant('annotations.title.publicCandidate')) {
                    annotatorRadioButtons = annotatorRadioButtons.concat('<div class="annotator-radioButton"><input class="annotator-radioButton radio-inline" name="scope" id="publicCandidate" type="radio" data-value="publicCandidate"/>&nbsp;' + buttonList[j] + '</div>');
                  }
                }
            }
            annotatorRadioButtons = annotatorRadioButtons.concat('</div>');
            input.append($(annotatorRadioButtons));
        }
        if (field.type === 'file') {
            input.addClass('annotator-attachment-label');
            element.append($('<div id = "annotationUploadContainer"><div flow-drop="" flow-btn="" id="dropZone" onclick="uploadAnnotationAttachment(event)" ondrop="drop(event)" ondragover="allowDrop(event)">\n' +
                '                <span class="dropTitle ng-binding">' + scope.translate.instant("annotations.title.selectOrDropFile") + '</span></div>\n' +
                '            <input id="uploadAnnotationFile" type="file" onchange="uploadingFile(event)" style="visibility: hidden;position: absolute;width: 300px;padding-bottom: 10px;height: 100px;padding-left: 0px;top: 10px;right: 0px;"></div>' + '<div id="fileDetail" style="height:10px; color: black;\n' +
                '    margin: 10px 0px 10px 10px;"><i class="fa fa-trash active-configuration-button" style="font-family:FontAwesome;position: absolute;" onclick="deleteAnnotationAttachment()" title="'+scope.translate.instant("annotations.title.removeAttachment")+'"></i><div id="annotationFileName" style="text-overflow: ellipsis;overflow: hidden;white-space:nowrap;display: inline-block;max-width: 255px;margin-left: 15px;height:14px;"></div></div>'));
        }

        if (field.type === 'date') {
            input.addClass('annotator-date-label');
            element.append($('<input id="annotator-date-input" style="display: inline;min-width: 50%" disabled/>'));
        }
    ...
    _setupDraggables: function () {
        ...
        var cornerItem;
        cornerItem = this.element.find('.annotator-header');
        if (cornerItem) {
            $('<span class="annotator-resize" style="cursor: ew-resize"></span>').appendTo(cornerItem);
        }

        var controls = this.element.find('.annotator-controls')[0],
            textarea = this.element.find('div').filter('#comments')[0],
...
Editor.template = [
    ...
    '    <div class="annotator-controls annotator-header"><span>'+ _t('annotation') + '</span></div>',
    '    <ul class="annotator-listing"></ul>',
    '    <div class="annotator-controls save-section">',
    '     <a href="#cancel" class="annotator-cancel">' + _t('Cancel') + '</a>',
    '      <a href="#save"',
    '         class="annotator-save annotator-focus">' + _t('Save') + '</a>',
    '    </div>',
    '    <div class="annotator-controls reviewer-section">',
    '     <a href="#cancel" class="annotator-cancel">' + _t('Cancel') + '</a>',
    '     <a href="#reject" class="annotator-reject">' + _t('Reject') + '</a>',
    '      <a href="#approve"',
    '         class="annotator-save annotator-focus">' + _t('Approve') + '</a>',
    '    </div>',
    '  </form>',
    '</div>'
].join('\n');
...
Highlighter.prototype.draw = function (annotation) {
    ...
    var normedRanges = [];
    var annotationElement = this.element;
    if(annotation.embeddedRef){
        annotationElement = annotation.publicationElement;
    }

    for (var i = 0, ilen = annotation.ranges.length; i < ilen; i++) {
        var r = reanchorRange(annotation.ranges[i], annotationElement);
        if (r !== null) {
            normedRanges.push(r);
        }
    }
	...
	/**
	 * Pinpoint - Comparison of quote before highlight content
	 */
	for (var j = 0, jlen = normedRanges.length; j < jlen; j++) {
        ...
        var isValidText = true;
 		...
 		var tempHighlightText = [];

 		var testText = null;

 		var testHighlightText = null;

        nodes = normed.textNodes();

		for (var k = 0, klen = nodes.length; k < klen; k++) {
    		var node = nodes[k];
    		tempHighlightText.push(node.nodeValue);
		}

		testText = annotation.quote.replace(/[^a-z\u00D1\u00F10-9]*/ig,'');

		testHighlightText = tempHighlightText.join('');

		var hightlightEncodeTest = testHighlightText.replace(/[^a-z\u00D1\u00F10-9]*/ig,'');

		if(hightlightEncodeTest !== testText) {
 			isValidText = false;
 		}
...
/**
 * Pinpoint: Allow only highlight annotation if GET command was successful
 */
 var isHighlightAllowed = false;
...
function main(options) {
    ...
    function start(app) {
        ...
        /**
         * Pinpoint: Highlight all annotations objects from the array
         */
        if(options.annotations !== null || options.annotations !== 'undefined') {
        	isHighlightAllowed = true;
        	if($(options.element).find("." + s.highlighter.options.highlightClass).length >0){s.highlighter.destroy();} //leftovers from switching context when opening an embedded ref
        ...
        s.textselector = new textselector.TextSelector(options.element, {
            onSelection: function (ranges, event) {
                if (ranges.length > 0) {
                    ...
                    /**
                     * Pinpoint: Allow Adder to load only on the closest of .pp-ui-widget-content or .pp-ui-widget-graphic widget.
                     */
                    if($(event.target).closest('.pp-ui-widget-' + options.source).length) {
        ...
        s.viewer = new viewer.Viewer({
            ...
            permitEdit: function (ann) {
                ...
                if(ann.scope === "public"){
                    return scope.vm.canWriteAnnotations() && scope.vm.isOwnerOrganization();
                } else if(ann.scope === "organization"){
                    return scope.vm.canWriteAnnotations();
                } else {return true;}
            },
    ...
    return {
        ...
        embeddedAnnotationsLoaded: function (anns) { 
            isHighlightAllowed = true;
        	for (var i = 0, len = anns.length; i < len; i++) {
                var anno = anns[i];
                s.highlighter.draw(anno);
            }
        	isHighlightAllowed = false;
        },
        ...
        annotationCreated: function (ann) { 
        	...
        	/** 
        	 * Pinpoint: Don't allow to highlight annotation
        	 */
    ...
    if (typeof this.element.ownerDocument !== 'undefined' &&
        ...
        $(this.document.body)
            .on("mouseup." + TEXTSELECTOR_NS, function (e) {
              var embann = $(e.target).parents('.embededContainer')[0];
              if (!embann){
...
TextSelector.prototype._checkForEndSelection = function (event) {
    ...
    //stop quill events for text selection from annotation comment section.
    if($(event.target).parents('.ql-editor, .ql-toolbar').length > 0 ) {
      event.preventDefault();
      //focus back to quill editor after click quill toolbar button
      setTimeout(function() {
          if ($(event.target).parents('.annotator-marginviewer-comment').length > 0) {
              $('.annotator-marginviewer-comment .ql-editor').focus();
          } else if ($(event.target).parents('#graphic-annotator-editor').length > 0) {
              $('#graphic-annotator-editor .ql-editor').focus();
          } else {
              $('#comments .ql-editor').focus();
          }
      },10);
      return;
    }
...
var Viewer = exports.Viewer = Widget.extend({
    ...
    constructor: function (options) {
        ...
        Viewer.itemTemplate = [
            ...
            '    <button type="button"',
            '            title="' + _t(scope.translate.instant('annotations.title.edit')) + '"',
            '            class="annotator-edit">' + _t(scope.translate.instant('annotations.title.edit')) + '</button>',
            '    <button type="button"',
            '            title="' + _t(scope.translate.instant('annotations.title.close')) + '"',
            '            class="annotator-delete">' + _t(scope.translate.instant('annotations.title.close')) + '</button>',
            '  </span>',
            '</li>'
        ].join('\n');
        ...
        this.isEmbeddedAnn = false;
        this.render = function (annotation) {

            if (annotation.text) {
                return annotation.text;
            } else {
                return "<i>" + _t(scope.translate.instant('annotations.alert.noComment')) + "</i>";
            }
        };
         ...
         * Pinpoint
         */
        this.renderTitle = function (annotation) {

            if (annotation.title) {
                return util.escapeHtml(annotation.title);
            } else {
                return "<i>" + _t(scope.translate.instant('annotations.alert.noTitle')) + "</i>";
            }
        };
        /**
         * Add Reviewer comment
         */
        this.renderReviewerComment = function (annotation) {

            if (annotation.scope === "private" && annotation.reviewerComment && !scope.vm.canWriteAnnotations()) {
                return "<div class='viewReviewerComment'>" + util.escapeHtml(annotation.reviewerComment) + "</div>";
            } else {
                return "";
            }
        };
        
        this.renderScope = function(annotation){
            if (annotation.scope) {
                if(annotation.scope === "private"){
                    return scope.translate.instant('annotations.title.private');
                }else if(annotation.scope === "public") {
                    return scope.translate.instant('annotations.title.public');
                }else if(annotation.scope === "organization") {
                    return scope.translate.instant('annotations.title.organization');
                }else if(annotation.scope === "publicCandidate") {
                    return scope.translate.instant('annotations.title.publicCandidate');
                }
            } else {
                return scope.translate.instant('annotations.title.private');
            }
        ...
        this.renderDate = function(annotation) {
          if(annotation.updated){
            return scope.translate.instant("annotations.title.date") + annotation.updated;
          } else {
            return "<i>" + _t(scope.translate.instant('annotations.alert.noDate')) + "</i>";
          }
        };

        this.renderAttachment = function (annotation) {
            var fileName = escape(annotation.uploadedFileName);
            var annotationControllerScope = angular.element(document.getElementById("organizeDialog")).scope();
            var inprogressText=scope.translate.instant('annotations.title.uploadInProgress');
            var renderedfileName=(annotationControllerScope.isAnnotationInProgressFlag===annotation.id)?"<i style='cursor:none;'>"+inprogressText+"</i>":annotation.uploadedFileName;
            var fileNameElement="<a style='cursor:pointer;color: #337ab7!important;text-decoration: underline;' onClick='downloadFile(event," + annotation.id + "," + "&quot;" + fileName + "&quot;" + ")';>" + renderedfileName + "</a>"

            annotation.uploadedBase64Image=(annotationControllerScope.isAnnotationInProgressFlag===annotation.id)?"":annotation.uploadedBase64Image;

            if (annotation.attachmentDownloadable !== 'true' && !isBlank(annotation.uploadedFileName)) {
                return '<span class="annotator-alert" style="color:red; position: absolute; height:15px; width:15px;"></span>' +"<i style='margin-left: 20px;'>" + _t(scope.translate.instant('annotations.alert.attachmentNotReady')) + "</i>";

            }  else if (annotation.uploadedBase64Image) {
                var mimeType = annotation.uploadedFileName.substr((annotation.uploadedFileName.lastIndexOf('.') + 1)).toLowerCase();
                return "<img src='data:image/" + mimeType + ";base64, " + annotation.uploadedBase64Image + "'" + "style='max-width: 150px;max-height: 150px;'" + "/>\n" + "<div>" + fileNameElement;
            }
            else if (annotation.uploadedFileName) {
                return fileNameElement;
            }
            else {
                return "<i>" + _t(scope.translate.instant('annotations.alert.noAttachment')) + "</i>";
            }
        }
        ...
        if (this.options.defaultFields) {
            ...
            this.addField({
                load: function (field, annotation) {
                    ...
                    $(field).html("<div class='viewScope'>" + self.renderScope(annotation) + "</div>");
                }
            });
        	this.addField({
                load: function (field, annotation) {

                    $(field).html("<div class='viewTitle'>" + self.renderTitle(annotation) + "</div>");
                }
            });
            ...
            this.addField({
                load: function (field, annotation) {
                    ...
                    $(field).html("<div class='viewText ql-editor'>" + self.render(annotation) + "</div>");
                }
            });

            this.addField({
                load: function (field, annotation) {
                    $(field).html("<div class='viewAttachment'>" + self.renderAttachment(annotation) + "</div>");
                }
            });

            this.addField({
                load: function (field, annotation) {
                    $(field).html(self.renderReviewerComment(annotation));
                }
            });

            this.addField({
                load: function (field, annotation) {
                    $(field).html("<div class='viewDate'>" + self.renderDate(annotation) + "</div>");
                }
            });
        }
        ...
        if (this.options.autoViewHighlights) {
            ...
            $(this.options.autoViewHighlights)
                ...
                .on("touchend." + NS, '.annotator-hl', function () {
                    self._startHideTimer();
                })
				;
    ...
    _annotationItem: function (annotation) {
        ...
        var isEmbeddedAnn = this.isEmbeddedAnn;
    ...
    _onEditClick: function (event) {
        var annotationPanelScope = angular.element("[pp-annotations-panel]").scope();
        //Show alert,when trying to edit annotation on symlink publication
        if (annotationPanelScope.$root.selectedPublication !== null && annotationPanelScope.$root.selectedPublication.sourcePublicationId) {
          this.hide();
          annotationPanelScope.vm.showAlertForSymlinkPublication();
          return;
        }
    ...
    _onDeleteClick: function (event) {
    	 ...
    	 * Pinpoint
    	 */
    	if (window.confirm(_t('Delete annotation?'))) {
    		var item = $(event.target)
                .parents('.annotator-annotation')
                .data('annotation');
            this.hide();
    ...
    _onHighlightMouseover: function (event) {
        ...
        if ($(event.target).parents('.embededContainer')[0]) {
          this.isEmbeddedAnn = true;
        } else {
          this.isEmbeddedAnn = false;
        }
...
Viewer.itemTemplate = [
    ...
    '    <button type="button"',
    '            title="' + _t('Close') + '"',
    '            class="annotator-delete">' + _t('Close') + '</button>',
    '  </span>',
    '</li>'
].join('\n');
...
function mousePosition(event) {
	...
	if (event.originalEvent.changedTouches && event.originalEvent.changedTouches.length > 0 && event.originalEvent.changedTouches[0].pageX && event.originalEvent.changedTouches[0].pageY) {
		finalPosition = {
			top: event.originalEvent.changedTouches[0].pageY - offset.top,
			left: event.originalEvent.changedTouches[0].pageX - offset.left
		};
	}
