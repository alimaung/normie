...
var PDFObject = function (obj){
    ...
    hasReaderActiveX = function (){
        ...
        //if (window.ActiveXObject) {
            ...
            console.log("Error: ActiveX Object is required to be enabled to display PDF files in Adobe Reader.");
            alert("Error: ActiveX Object is required to be enabled to display PDF files in Adobe Reader.");
    ...
    embed = function(targetID){
        ...
        url = encodeFileNameWithHash(url);
        targetNode.innerHTML = '<object onreadystatechange=\'pdfLoadingComplete()\' onload=\'pdfLoadingComplete()\'   data="' + url +'" type="application/pdf" width="' + width + '" height="' + height + ' "></object>';
...
/**
 * Custom logic added to support file name with # character - rebuilds the url.
 *
 * @param url
 * @returns {*}
 */
function encodeFileNameWithHash(url) {
    var lastIndexSlash = url.lastIndexOf("/");
    var lastIndexPdf = url.indexOf("?");
    var fileName = url.substring(lastIndexSlash + 1, lastIndexPdf);
    if (fileName && fileName.indexOf("#") !== -1) {
        url = url.substring(0, lastIndexSlash + 1) + fileName.replace(/#/g, "%23") + url.substring(lastIndexPdf);
    }
    return url;
}