import re, os, csv, time, logging, win32api, win32con, keyboard, pythoncom, pyperclip, subprocess, pypdf as pp, win32gui as wg, win32com.client, pygetwindow as gw
...
logger = get_logger(name="my_application", level=logging.DEBUG, colored=True, file_path="logs/app.log", file_level=logging.INFO, max_file_size=10*1024*1024, backup_count=3                  )

EXCEL_FILES = {'verzeichnis': Path(r'C:\Users\u8064927\Desktop\Rolls-Royce X Ali\.coding\Normstelle\ChemScan\xls\#Verzeichnis.xlsb'), 'chemscan': Path(r'P:\k-z\Ofs\Normstelle\Teile-und-Stoffe\Chemscan\TKZ_AT&S_export_2024_04_24.xlsx')}
...
class ExcelDataProcessor:
    ...
    def open_chem(self, data: Dict[str, Any], index) -> None:
    ...
    def mark_row_as_done(self, row: int, processed_data) -> None:
        ...
        for line in processed_data:
            ats = line["ats_comment"]
            sdb = line["sdb_comment"]

            ats_id = ats.split("_")[1].replace("-", "/")
            sdb_id = sdb.split("_")[1].replace("-", "/")
        ...
        try:
            ...
            for wb in self.excel.Workbooks:
                if wb.name == EXCEL_FILES['chemscan'].name:
                    ...
                    active_sheet.Cells(row, 3).Value = ats_id
                    active_sheet.Cells(row, 4).Value = sdb_id
