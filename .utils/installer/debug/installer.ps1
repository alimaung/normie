...
function Start-ESEAutomation {
    ...
    try {
        # Configure process start info for console capture
        $ProcessInfo = New-Object System.Diagnostics.ProcessStartInfo
        
        # Check if we're using the PowerShell fallback
        if ($ESEBatPath.EndsWith('.ps1')) {
        ...
        $ProcessInfo.CreateNoWindow = $false  # Keep window visible
        ...
        # Send miniconda installation command
        Write-Info "Sending 'integ miniconda3' command..."
        ...
        } -ArgumentList $ESEProcess
        
        # Monitor with timeout (5 minutes max)
        $TimeoutMinutes = 5
        $StartTime = Get-Date
        
        while ((Get-Date) -lt $StartTime.AddMinutes($TimeoutMinutes) -and !$InstallationComplete) {
            # Check for new output
            $JobOutput = Receive-Job -Job $ReaderJob -Keep
            if ($JobOutput) {
        ...
        # Cleanup job
        Remove-Job -Job $ReaderJob -Force
...
function Start-RRPYToolsAutomation {
    ...
    try {
        ...
        # Send rrpytools fix command
        Write-Info "Sending 'rrpytools fix' command..."
        ...
        # Monitor with timeout (3 minutes max)
        $TimeoutMinutes = 3
        $StartTime = Get-Date
